...
`uv sync --extra portraits --extra aws`. Both extras in one command: `uv sync
--extra aws` alone removes `portraits`, which took a run down after the case was
paid for (D-123).
...
`FileShelf` and `S3Shelf` are both written and both satisfy `Shelf`. Which one
a process uses is `MYSTERY_BUCKET`: set means S3, unset means the folder, and
`shelf()` is the only line that knows both exist. Seventeen tests against a fake
bucket that paginates; `scripts/s3_check.py` is the by-hand check against real
AWS. boto3 is the `aws` extra and nothing in the suite imports it. See D-118.

Both entry points now pick a shelf at the top of `main` and print
`Shelf: s3://...` when one is configured, so `MYSTERY_BUCKET` genuinely moves
where cases live. Asserted by running the real entry points, not by mocking
(D-119).

    set MYSTERY_BUCKET=mystery-cases-197099231733-eu-north-1-an
    uv run python -m mystery.cli --dry-run

The bucket now holds both: `cases/` and `index/` for the shelf, `art/` for the
pictures. A portrait is served as a presigned S3 link and a 307, so a megabyte
and a half never goes through the application (D-121).

Sessions and the rota are in DynamoDB (D-122), chosen by their own variables:
...
`mystery-sessions` has TTL on `expires`; `mystery-rota` gets the conditional
write `claim` was specified around in D-079. Both on-demand, both eu-north-1.

Next: Lambda, Terraform and CI. The budget guard is not optional.
...
- `gallery.py` — where the pictures live: a folder, or the bucket under
  `art/`, served to the browser as a presigned link rather than through the app
...
- `palette.py` — the commission, the occasion, where on earth the house is,
  manners, voices, motives, intrigues, the standing, the old business, the
  murder hour and the clock, all dealt from the seed
...
- Five hundred and four tests green, ruff clean, no test calls an API
- Architecture note: https://claude.ai/code/artifact/537fe482-a219-4b13-a108-062bff885a1f
- One hundred and forty six decisions in `docs/decisions.md`
...
| A20 | Somebody innocent lies about the hour the killer lies about | reports |
| A21 | Something moved, somebody saw it, and not everybody did | reports |
| A22 | One scene told two ways, and one falsehood that is an honest mistake | reports |
...
**How to check the state of things, cheapest first**
    ...
    uv run pytest                             # 504 tests, no network, no spend
    uv run ruff check .
